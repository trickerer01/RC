...
from rex import (
    re_replace_symbols, re_wtag, re_idval, re_uscore_mult, re_not_a_letter, re_bracketed_tag, re_numbered_or_counted_tag,
    re_or_group, re_neg_and_group, re_tags_to_process, re_tags_exclude_major1, re_tags_exclude_major2, re_tags_to_not_exclude,
    ...
    if len(extra_tags) > 0:
        if is_extra_seq:
            ...
            if idi not in id_sequence:
                suc = False
                Log.trace(f'{sfol}Album {sname} isn\'t contained in id list \'{str(id_sequence)}\'. Skipped!',
        ...
        for extag in extra_tags:
            if extag[0] == '(':
                ...
                if is_neg_and_group_matches(extag, tags_raw):
                    suc = False
                    Log.info(f'{sfol}Album {sname} contains excluded tags combination \'{extag[1:]}\'. Skipped!',
