...
class ThrottleChecker:
        ...
        try:
            while True:
                ...
                if not dwn.is_writing(dest):  # finished already
                    Log.error(f'ThrottleChecker: {self._ii.my_shortname} checker is still running for finished download!')
                    break
                if self._response is None:
                    Log.debug(f'ThrottleChecker: {self._ii.my_shortname} self._response is None...')
                    continue
                ...
                if file_size < last_size + slow_con_dwn_threshold:
                    Log.warn(f'ThrottleChecker: {self._ii.my_shortname} check failed at {file_size:d} ({last_speed:.2f} KB/s)! '
    ...
    def __str__(self) -> str:
        return f'{self._ii.my_shortname} (orig size {self._init_size}): {", ".join(self._speeds)}'
