...
from defs import (
    LoggingFlags, UNTAGGED_POLICIES, DOWNLOAD_POLICY_DEFAULT, DOWNLOAD_POLICY_ALWAYS, ACTION_STORE_TRUE, PREFIX,
)
        ...
        for query_raw in fmt_str.split('; '):
            try:
                ...
                self._add_subquery(SubQueryParams(
                    subfolder, parsed.extra_tags, parsed.minimum_score, parsed.minimum_rating, parsed.untagged_policy,
                    id_sequence
                ))
            except Exception:
                import traceback
            ...
            if not is_filtered_out_by_extra_tags(idi, tags_raw, sq.extra_tags, sq.id_sequence, sq.subfolder):
                ...
                for vsrs, csri, srn, pc in zip((score, rating), (sq.minscore, sq.minrating), ('score', 'rating'), ('', '%')):
                    if len(vsrs) > 0 and csri is not None and sq_skip is False:
                        try:
                            if int(vsrs) < csri:
                                Log.info(f'[{sq.subfolder}] Album {sname} has low {srn} \'{vsrs}{pc}\' (required {csri:d})!',
